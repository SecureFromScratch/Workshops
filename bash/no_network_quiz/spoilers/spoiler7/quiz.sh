...
# Function to check for valid characters in the input
hasOnlyValidChars() {        
    if echo "$1" | grep -qvE "^[${VALID_CHARS}]+$"; then        
        return 1 # false: invalid characters found
    else        
        return 0 # true: only valid characters
    fi
}
...
askForName() {    
    while true; do    
        ...
        elif ! hasOnlyValidChars "$name"; then
            echo "illegal name!"        
        elif [[ $name  == *" "* ]]; then                      
            break
        else        
