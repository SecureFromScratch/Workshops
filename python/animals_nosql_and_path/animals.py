...
app = FastAPI()
tls_cert = 'X509-cert-7418278991288180999.pem'
...
# list all entities in the "animals" collection
@app.get("/api/mongo/animals")
async def get_animals():
    collection = db[ANIMALS_COLLECTION]
    animals = await collection.find({}, {'_id': 0}).to_list(length=None)
    return animals
# list all entities in the sayings collection (which is currently "english")
@app.get("/api/mongo/animals")
async def get_animals():
    collection = db[SAYINGS_COLLECTION]
    sayings = await collection.find({}, {'_id': 0}).to_list(length=None)
    return sayings
#---------------------------------------------------------------------
