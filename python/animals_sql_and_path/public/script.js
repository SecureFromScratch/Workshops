...
async function fetchAnimals() {
	...
	animals.forEach(animal => {
        ...
        button.style.backgroundImage = `url('./${animal.imgName}')`;
...
async function fetchLanguages() {
    ...
    // Remove existing children from the langSelector
    while (langSelector.firstChild) {
        langSelector.removeChild(langSelector.firstChild);
    }

	languages.forEach(lang => {
        const option = document.createElement('option');
        option.value = lang.table;
...
async function showAnimal() {
    const response = await fetch(`http://localhost:5001/api/myanimal?color=${selectedColor}&imgName=${selectedAnimalData.imgName}`);
...
async function showSaying() {
    const langSelector = document.getElementById('languages');
    const langTable = langSelector.value;
    ...
    console.log("asking for saying " + selectedAnimalData.animalId);
    const response = await fetch('http://localhost:5001/api/animalsays', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
        body: JSON.stringify({ langTable: langTable, color: selectedColor, animalId: selectedAnimalData.animalId }),
    });
